...
opm index add \
  ...
  --tag docker.io/$DOCKER_HUB_REPOSITORY/deploy-operator-index:26.3 \
  --generate
docker build -f index.Dockerfile -t docker.io/$DOCKER_HUB_REPOSITORY/deploy-operator-index:26.3 .
docker push docker.io/$DOCKER_HUB_REPOSITORY/deploy-operator-index:26.3
