...
opm index add \
  --bundles docker.io/$DOCKER_HUB_REPOSITORY/deploy-operator-bundle:$RELEASE_APP_EXPLICIT \
  --tag docker.io/$DOCKER_HUB_REPOSITORY/deploy-operator-index:24.3 \
  --generate
docker build -f index.Dockerfile -t docker.io/$DOCKER_HUB_REPOSITORY/deploy-operator-index:24.3 .
docker push docker.io/$DOCKER_HUB_REPOSITORY/deploy-operator-index:24.3
