...
opm index add \
  ...
  --from-index docker.io/$RELEASE_REGISTRY/deploy-operator-index:24.3 \
  --tag docker.io/$RELEASE_REGISTRY/deploy-operator-index:24.3 \
  --generate
docker build -f index.Dockerfile -t docker.io/$RELEASE_REGISTRY/deploy-operator-index:24.3 .
docker push docker.io/$RELEASE_REGISTRY/deploy-operator-index:24.3
