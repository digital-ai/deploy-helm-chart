...
opm index add \
  ...
  --tag docker.io/$RELEASE_REGISTRY/deploy-operator-index:25.1 \
  --generate
docker build -f index.Dockerfile -t docker.io/$RELEASE_REGISTRY/deploy-operator-index:25.1 .
docker push docker.io/$RELEASE_REGISTRY/deploy-operator-index:25.1
